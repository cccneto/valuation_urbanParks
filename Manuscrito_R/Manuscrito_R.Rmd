...
tidy.SemiParBIV <- function(x, conf.level = 0.95, ...) {
    s <- summary(x, prob.lev = 1 - conf.level, ...)
    s <- s[grepl("tableP", names(s))]
    s <- s[sapply(s, is.null) == FALSE]
    for (i in seq_along(s)) {
        colnames(s[[i]]) <- c("estimate", "std.error", "statistic", "p.value")
        s[[i]] <- data.frame(s[[i]])
        s[[i]]$equation <- paste("Equation", i)
        s[[i]]$term <- row.names(s[[i]])
    }
    out <- bind_rows(s)
    out
}

glance.SemiParBIV <- function(x, ...) {
  out <- data.frame(
    nobs = x$n,
    robust = x$robust)
  out
...
modelsummary(bvp.santana, output = "markdown", group = equation + term ~ model)
```
...
bvp.sitiotrindade, bvp.trezedemaio, 
bvp.santosdumont
bvp.lindu, bvp.caiara, bvp.macaxeira 
bvp.jaqueira, bvp.santana 