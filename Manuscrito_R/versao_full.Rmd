...
authors: "Carlos Eduardo de Menezes Silva, Claudiano Carneiro da Cruz Neto ... "
output: 
  pdf_document:
...
The MVC dichotomous choice format requires a qualitative choice model. The use of linear distribution of the WTP and a Bivariate Probit Model (BPM), was developed based on the model by [@cameron1994estimation]. We assumed that the error of the second dichotomous question is correlated with the error of the first question. For this reason, [@alberini1995optimal] recommendation for the choice of bivariate dichotomous models was used, because if the coefficient correlation, $\rho \ne 1$ , it is clear that, in general, the second WTP does not perfectly match the first and can be interpreted as a revised version of the amount of the first WTP. If WTP values are independently determined, then $\rho = 0$. For all other values of the correlation coefficient, the interval $0 < \rho < 1$ is valid, which implies that the correlation between the two WTP values is less than perfect.
...
\label{tab2}
    ...
    Facilities (good = 1, ) & 0 & 0  & 1 & 0.5552 & 1 & 1 \tabularnewline
    Temperature & 0 & 0  & 0  & 0.3187 & 1 & 1 \tabularnewline
...
base %>%
   select(resp1, resp2, lance1, lance2, idade, sexo, escolar, infraestrutura, temperatura, renda) %>%
  rename(
    "Age" = idade,
    "Gender" = sexo,
    "School"= escolar,
   ...
   "First Bid" = lance1,
    "Second Bid" = lance2,
     "First Response" = resp1,
     "Second Response" = resp2)
...
\begin{table}[!htp]
\centering
\caption{Characteristics of access and use of urban parks studied.}
\scriptsize
\begin{tabular}{lllll}
\multicolumn{2}{c}{\textbf{Travelled distance}} &\textbf{} &\multicolumn{2}{c}{\textbf{Objective}} 
greater 4500m &31.49\% & &Study &1.09\% \\
between 3001 and 4500m &4.34\% & &leisure &25.33\% \\
between 1500 and 3000m &20.63\% & &Physical activity &36.96\% \\
between 1001 and 1500m &5.97\% & &Children &27.83\% \\
between 500 and 1000m &9.66\% & &Others &8.80\% \\
between 100 and 500m &27.90\% & & & \\
\multicolumn{2}{c}{\textbf{Length of time spent there}} & \multicolumn{2}{c}{\textbf{Physical Activity length}} \
Up to 1h &43.21\% & &up to 30 min &50.92\% \\
between 1 e 2h &34.09\% & &between 30 e 60 min &24.54\% \\
between 2 e 2,5h &9.55\% & &between 60 e 120 min &16.29\% \\
between 2,5 e 3h &8.36\% & &more than 120 min &8.25\% \\
more than 3h &4.78\% & & & \\
\multicolumn{2}{c}{\textbf{Visiting Frequency}} & & & \
1x (once) /month &20.94\% & & & \\
between 2 e 6x /month &60.20\% & & & \\
between 6 e 11x /month &7.09\% & & & \\
between 11 e 19x/month &4.03\% & & & \\
between 20 e 30x /month &7.74\% & & & \\
\bottomrule
\end{tabular}
\end{table}
```
...
The results from the BP regression for the general sample are shown in Table 3. The estimations for Eq. (1) are shown in the upper part of the Table, while the estimation for Eq. (2) in the lower part. The probability that WTP is equal or higher to the two bids amount is influenced by the respondent's own characteristics and by a series of independent variables that reflect their preferences for the ES being valued. The coefficient on the bid was negative and statistically significant in both equations, which indicates that as the price increased, the probability of a positive answer to the WTP question decreased.
...
```{=tex}
\begin{table}[!tbp]
\caption{Willingness to pay for Park visitors} \label{tab3}
    \begin{center}
    \begin{tabular}{lrrrr}
    \hline\hline
...
```{r figurename, echo=FALSE, fig.cap= "Variation in expenses with the implementation and maintenance of Urban Parks in the municipality of Recife - Prepared by the authors based on data from the Transparency Portal of the City of Recife" , out.width = '70%', fig.align="center"}
...
Furthermore, the significance of variables such as temperature and infrastructure for PAD underscores the importance of adequate afforestation of these spaces for their proper use. The maintenance of these areas is necessary and requires investments by the government.
