...
baseunificada <- read_excel("C:/Users/User/Desktop/dados_cadu_t4/dados_jaqueira_limpo.xlsx")
...
mod1 <- glm(treat.eq, family = "binomial", data = dados_jaqueira)
summary(mod1)
...
sexo_med <- mean(as.numeric(dados_jaqueira$sexo))
escolar_med <- mean(as.numeric(dados_jaqueira$escolar))
temperatura_med <- mean(as.numeric(dados_jaqueira$temperatura))
infra_med <- mean(as.numeric(dados_jaqueira$infraestrutura))
lance2_med <- mean(dados_jaqueira$lance2)
freqvis_med <- mean(as.numeric(dados_jaqueira$freqvis))
tempoestad_med  <- mean(dados_jaqueira$tempoestad)
saudefis_med  <- mean(as.numeric(dados_jaqueira$saudefis))
saudemen_med  <- mean(as.numeric(dados_jaqueira$saudemen))
sombra_med <- mean(as.numeric(dados_jaqueira$sombra))
...
cf_freqvis <- bvp_jaqueira$coefficients[6]
cf_tempoestad <- bvp_jaqueira$coefficients[7]
cf_saudefis <- bvp_jaqueira$coefficients[8]
cf_saudemen <- bvp_jaqueira$coefficients[9]
cf_sombra <- bvp_jaqueira$coefficients[10]
cf_temperatura <- bvp_jaqueira$coefficients[11]


# coeficientes eq 2
cf_intercept2 <- bvp_jaqueira$coefficients[12]
cf_lance2 <- bvp_jaqueira$coefficients[13]
cf_idade2 <- bvp_jaqueira$coefficients[14]
cf_sexo2 <- bvp_jaqueira$coefficients[15]
cf_escolar2 <- bvp_jaqueira$coefficients[16]
cf_freqvis2 <- bvp_jaqueira$coefficients[17]
cf_tempoestad2 <- bvp_jaqueira$coefficients[18]
cf_saudefis2 <- bvp_jaqueira$coefficients[19]
cf_saudemen2 <- bvp_jaqueira$coefficients[20]
cf_sombra2 <- bvp_jaqueira$coefficients[21]
cf_temperatura2 <- bvp_jaqueira$coefficients[22]
