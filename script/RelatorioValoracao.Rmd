...
dap_eq1_L <- -(cf_intercept + cf_idade*idade_med + cf_sexo*sexo_med + cf_escolar*escolar_med + cf_temp*temp_med + cf_infra*infra_med)/cf_lance1 # R$ 44,53
...
### Dap equação 2
```{r}
dap_eq2_L <- -(cf_intercept + cf_idade2*idade_med + cf_sexo2*sexo_med + cf_escolar2*escolar_med + cf_temp2*temp_med + cf_infra2*infra_med)/cf_lance2 # R$ 48,16 

print(dap_eq2_L)
```

# Configurando listas de variáveis - modelo com lnRendat
...
# DAP's MODELO LOG LINEAR NA RENDA

## coeficientes
```{r}
# Equação 1
cf_intercept <- bvp_ll$coefficients[1]
cf_lance1 <- bvp_ll$coefficients[2]
cf_idade <- bvp_ll$coefficients[3]
cf_sexo <- bvp_ll$coefficients[4]
cf_escolar <- bvp_ll$coefficients[5]
cf_lnRendat1 <- bvp_ll$coefficients[6]
cf_temp <- bvp_ll$coefficients[7]
cf_infra <- bvp_ll$coefficients[8]
ct_t1 <- bvp_ll$coefficients[2]
...
```{r}
# equation 2
cf_intercept2 <- bvp_ll$coefficients[9]
cf_lance2 <- bvp_ll$coefficients[10]
cf_idade2 <- bvp_ll$coefficients[11]
cf_sexo2 <- bvp_ll$coefficients[12]
cf_escolar2 <- bvp_ll$coefficients[13]
cf_lnRendat2 <- bvp_ll$coefficients[14]
cf_temp2 <- bvp_ll$coefficients[15]
cf_infra2 <- bvp_ll$coefficients[16]
ct_t1 <- bvp_ll$coefficients[10]

```

### Dap equação 1
```{r}
dap_eq1_ll <- renda_med - (renda_med*(exp(-((cf_intercept + cf_idade*(idade_med) + cf_sexo*sexo_med + cf_escolar*escolar_med + cf_temp*temp_med + cf_infra*infra_med)))/cf_t1))
...
### Dap equação 2
```{r}
dap_eq2_ll <- renda_med - (renda_med*(exp(-cf_intercept2 + cf_idade2*idade_med + cf_sexo2*sexo_med + cf_escolar2*escolar_med + cf_temp2*temp_med + cf_infra2*infra_med)/cf_t2))
